...
# inverse projection formula from x, y to longitude, latitude using lambert conical conformal projection
def inverse_lambert(x, y, lambda_0, phi_0, phi_1, phi_2):
    # compute parameters
    phi_1_rad = math.radians(phi_1)
    phi_2_rad = math.radians(phi_2)
    phi_0_rad = math.radians(phi_0)
    lambda_0_rad = math.radians(lambda_0)
    n = math.log(math.cos(phi_1_rad) / math.cos(phi_2_rad)) / math.log(
        math.tan((math.pi/4) + (phi_2_rad/2)) / math.tan((math.pi/4) + (phi_1_rad/2)))
    F = math.cos(phi_1_rad) * \
        math.pow(math.tan((math.pi/4) + (phi_1_rad/2)), n) / n
    rho_0 = F * (1.0/math.pow(math.tan((math.pi/4) + (phi_0_rad/2)), n))
    # compute rho and theta
    rho = math.copysign(1, n) * math.sqrt(math.pow(x, 2) +
                                          math.pow(rho_0 - y, 2))
    theta = math.atan(x / (rho_0 - y))

    # compute latitude and longitude
    phi = 2 * math.atan(math.pow(F / rho, 1/n)) - (math.pi/2)
    lambda_proj = lambda_0_rad + (theta / n)
    lambda_ = math.degrees(lambda_proj)
    phi_ = math.degrees(phi)
    return phi_, lambda_

### READ INPUT ###
cwd = os.getcwd()
mwd = "{c}\\Vrp-Set-A\\A".format(c=cwd)
owd = "{c}\\haryana\\lambert\\T".format(c=cwd)
print(mwd)
l = os.listdir(mwd)
lnew = []
for f in l:
...
for file_name in lnew:
    ...
    for i in range(no_of_cust + 1):
        lati, longi = inverse_lambert(coords[i].lon,
                                      coords[i].lat,
                                      82.5,  # lambda_0 - central meridian
                                      8,  # lphi_0 - latitude of origin
                                      17.7,  # phi_1,
                                      27.3  # phi_2
                                      )
    ...
    if (map):
        lambert_map = folium.Map()
        lambert_map = folium.Map(
            center=[currCenter.lon, currCenter.lat], zoom_start=0.5)
        folium.CircleMarker(location=[currCenter.lon, currCenter.lat],
                            radius=1,
                            weight=5).add_to(lambert_map)
        for c in lat_lon_list:
            folium.CircleMarker(location=[c.lon, c.lat], fill_color='#FF0000',
                                radius=1,
                                weight=5).add_to(lambert_map)
        lambert_map.fit_bounds(lambert_map.get_bounds())
        lambert_map.save("lambert_map.html")
        webbrowser.open("lambert_map.html")

    # calculating "shift"
    geolocator = Nominatim(user_agent="MyApp")
    city_loc = geolocator.geocode("Haryana") #input desired city's name here
    cityCenter = Location((city_loc.longitude), (city_loc.latitude))
    #cityCenter = Location((80.274696), (13.082258)) - manual input for desired center 
    ...
    # city boundary as a geodataframe - boundary as MultiPolygons
    # shp = ox.geocode_to_gdf("Chennai, TamilNadu, India")
    # Multi = shp.loc[0, 'geometry']

    # boundary coords obtained as a single polygon from the internet - manually
